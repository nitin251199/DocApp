...
router.get('/fetchallappointments', function (req, res, next) {
  ...
  router.post('/fetchappointmentsbydoctor', function (req, res, next) {
    ...
    pool.query("select * from appointments where doctorname=?", [req.body.doctor], function (error, result) {
      if (error) {
  ...
  router.post('/fetchappointmentsbyuser', function (req, res, next) {
    ...
    pool.query("select * from appointments where phoneno=?", [req.body.phoneno], function (error, result) {
      if (error) {
        console.log("ERROR",error)
        res.status(500).json([])
      }
  ...
  router.post('/insertappointments', function (req, res, next) {
    pool.query("insert into appointments (patientname, date, department, doctorname, phoneno, message, time, amount) values(?,?,?,?,?,?,?,?)", [req.body.patientname, req.body.date, req.body.department, req.body.doctor, req.body.phoneno,req.body.message,req.body.time,req.body.amount], function (error, result) {
      if (error) {
  ...
  router.post('/updateappointments', function (req, res, next) {
    ...
    pool.query("update appointments set patientname=?, date=?, department=?, doctorname=?, phoneno=?, message=?, time=?, amount=? where appointmentid=?", [req.body.patientname, req.body.date, req.body.department, req.body.doctor, req.body.phoneno, req.body.status, req.body.phoneno, req.body.message,req.body.time,req.body.amount, req.body.appointmentid], function (error, result) {
      if (error) {
  ...
  router.post('/deleteappointments', function (req, res, next) {
    pool.query("delete from appointments where appointmentid=?", [req.body.appointmentid], function (error, result) {
      if (error) {
        res.status(500).json(false)
      }
      else {
